...
저희 팀의 개발 목표는 싸피 교육 과정에서 배운 내용을 융합하기 위해서 노력하였습니다. 프론트 엔드는 `vue.js`를 사용해 웹의 구조를 설계하였고, 디자인 라이브러리에 의존하지 않기 위해서  `css`,와` js`만으로 모든 ui 및 인터렉션 을 구현했습니다. 또한 얼굴 감정 인식, `TTS`,`STT`서비스를 js로 구현해서 적용하였고, `AR 이모지`와  `webrtc` 서버를 직접 구현했습니다. 또한 사용자들이 서비스에 흥미를 가지고 접근할 수 있게 가상 세계를 모티브로 한  ui를 제공하기 위해서 `three.js`,`webGL`를 활용하여 `3D ui`를 제작하여 서비스를 제작했습니다.
