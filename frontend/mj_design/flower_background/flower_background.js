const branchesRandomOrder = document.querySelector('[id^=BranchGroup]').toArray().sort(function(){return 0.5-Math.random()});
const branchesRandomOrderLeft = document.querySelector('[id^=BranchGroup-left]').toArray().sort(function(){return 0.5-Math.random()});
const branchesRandomOrderRight = document.querySelector('[id^=BranchGroup-right]').toArray().sort(function(){return 0.5-Math.random()});
const branchesRandomOrderBottom = document.querySelector('[id^=BranchGroup-bottom]').toArray().sort(function(){return 0.5-Math.random()});
...
function flowersBloom() {
  ...
  const currentBranch = document.querySelector(this.target);
  const petals = currentBranch.querySelector('[id^=petal-]');
  const flowers = currentBranch.querySelector('[id^=flower-]');
  const buds = currentBranch.querySelector('[id^=bud-]');
  const blooms = currentBranch.querySelector('[id^=bloom-]');
...
function smallBranchesSway() {
  const smallBranches = document.querySelector('[id^=smallbranch-group]').toArray();
  const tl = new TimelineMax({yoyo: true, repeat: -1});
