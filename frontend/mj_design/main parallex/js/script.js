...
function scrollFunc(e) {
    ...
    for(var i=0; i< totalNum ; i++){
        imageAll[i].style.transform = "perspective(400px) translateZ("+ scrollTop/(50*(totalNum-i)) +"px) translateY("+scrollTop / (11*(i))+"px) scale("+(1+scrollTop / (5*(totalNum-i))*0.001 )+") " ;
...
//        console.log(scrollTop , 1+scrollTop / (5*(totalNum-i))*0.001 );
      if (i === 1 ){
        imageAll[i].style.transform = "perspective(400px) translateZ("+ scrollTop/(30*(totalNum-i)) +"px) translateY("+-scrollTop / (5*(totalNum-i))+"px) scale("+(1+scrollTop / (5*(totalNum-i))*0.009 )+") translateX("+-(1+scrollTop / (5*(totalNum-i))*0.015 )+"px)" ;
      }
      if(i == 6){
        imageAll[i].style.transform = "perspective(400px) translateZ("+ scrollTop/(35*(totalNum-i)) +"px) translateY("+-scrollTop / (40*(totalNum-i))+"px)" ;
      }
    }
